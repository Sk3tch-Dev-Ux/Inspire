...
import { validateState, exchangeDiscordCode } from '@/lib/oauth';
...
export async function GET(request: NextRequest) {
  ...
  if (error || !code || !state) {
    return NextResponse.redirect(new URL('/login?error=discord_denied', APP_URL));
  }
  ...
  try {
    ...
    return NextResponse.redirect(new URL('/login?error=discord_callback_failed', APP_URL));
  }
}
